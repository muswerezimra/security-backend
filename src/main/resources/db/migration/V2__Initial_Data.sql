...
INSERT INTO department (name, description, code, created_by, created_date)
VALUES ('Information Technology', 'IT Department', 'IT_DEPT', 'admin', CURRENT_TIMESTAMP)
ON CONFLICT DO NOTHING;

-- Insert Sample User System
INSERT INTO user_system (name, description, status, created_by, created_date)
...
SELECT 'TaRMS_User', id, 'admin', CURRENT_TIMESTAMP
FROM user_system
WHERE name = 'TarmsServiceBus'
  AND NOT EXISTS (
      SELECT 1 FROM roles r
      JOIN user_system s ON r.system_id = s.id
      WHERE r.name = 'TaRMS_User' AND s.name = 'TarmsServiceBus'
  );

INSERT INTO roles (name, system_id, created_by, created_date)
SELECT 'TaRMS_Dev', id, 'admin', CURRENT_TIMESTAMP
FROM user_system
WHERE name = 'TarmsServiceBus'
  AND NOT EXISTS (
      SELECT 1 FROM roles r
      JOIN user_system s ON r.system_id = s.id
      WHERE r.name = 'TaRMS_Dev' AND s.name = 'TarmsServiceBus'
  );
...
VALUES (
    ...
    TRUE, FALSE, (SELECT id FROM department WHERE code = 'IT_DEPT'), 'admin', CURRENT_TIMESTAMP
)
